...
test("mounted shell keeps save, recovery, conflict, lease, migration, and compaction decisions truthful",
  async (t) => {
    ...
    dom.window.scpefe = {
      ...
      enterEditMode: async ({ forceTakeover }) => {
        calls.push(["edit", forceTakeover]); editAttempts += 1;
        if (!forceTakeover && editAttempts <= 2) {
          return { decisionRequired: "lease-takeover", holderName: "Remote editor" };
        }
        return { ...base, readOnly: false };
      },
      restoreRecoveredWork: async () => ({ ...base, content: "recovered private text",
        readOnly: false, recoveredUnsaved: true, cursor: { start: 3, end: 3 } }),
      discardRecoveredWork: async () => { recoveryDiscards += 1; return base; },
      ...
      reconnectPendingPublication: async () => {
        ...
        return { content: "offline manual save",
          publicationState: publicationAttempts === 1 ? "pending-publication" : "conflict" };
      },
      beginDivergenceResolution: async () => ({ content: "merged exact text",
        hasConflicts: false, ancestorRevision: "11".repeat(32),
        localRevision: "22".repeat(32), currentRevision: "33".repeat(32) }),
      saveDivergenceResolution: async (content) => ({ saved: true, content,
        publicationState: "target-published" }),
      discardPendingPublication: async () => base,
      acceptHeadMismatch: async () => { headAccepts += 1; return base; },
      ...
      migrateDocument: async ({ forceTakeover }) => {
        calls.push(["migrate", forceTakeover]); migrationAttempts += 1;
        if (!forceTakeover) {
          return { decisionRequired: "lease-takeover", holderName: "Future clock holder" };
        }
    ...
    assert.equal(recoveryDiscards, 1);
    ...
    await ui.waitFor(() => assert.equal(headAccepts, 1));
    await command("Security", "Lock");
